        ...
        SECURITY_PAYLOAD_TYPE_SESSION_HELLO)
    {
            ...
            sizeof(mathos_session_hello_t))
        {
            ...
            printf(
                ...
                packet.payload_len,
                (unsigned int)sizeof(mathos_session_hello_t));
        ...
        SECURITY_PAYLOAD_TYPE_SESSION_CONFIRM)
    {
            ...
            sizeof(mathos_session_confirm_t))
        {
            ...
            printf(
                ...
                packet.payload_len,
                (unsigned int)sizeof(mathos_session_confirm_t));
...
static bool rc_session_hello_send_once(void)
{
    ...
    */
    ...
    uint8_t frame[MATHOS_WIRE_MAX_FRAME_LEN];
...
#if FEATURE_SECURE_GATEWAY_MODE
        /*
            Normal RC control packets are forbidden until
            SESSION_CONFIRM has been authenticated and the
            directional session keys exist.
        */
        if (!rc_operational_session_ready ||
            !mathos_secure_session_keys_are_set())
        {
        ...
        if (security_wrap_rc_packet(&packet, &secure_packet, read_link_mode_switch()))
        {
...
#if DEBUG_SECURE_WIRE_TEST
            {
                ...
                if (encode_status == MATHOS_STATUS_OK)
                {
                    ...
                    if (decode_status == MATHOS_STATUS_OK)
                    {
                        decrypt_status = mathos_secure_decrypt_session_packet(&decoded_wire_packet);
